    ...
    <nav className="shadow-lg bg-background fixed w-full h-12 md:h-14 lg:h-16 px-3 md:px-4 lg:px-6 py-2 md:py-3 flex items-center justify-between z-50">
      ...
      <div className="flex-1 flex justify-center px-2 md:px-4 max-w-md mx-auto">
        <div className="relative w-full">
          ...
          <div className="absolute top-full bg-red-500 w-full h-fit">
            {isLoading && <LoaderCircle />}
            {isSearchResultActive && data?.data?.map(({ companyId, companyArabicName }) => {
              return (
                <span
                  key={companyId}
                  className="p-2 border rounded-2xl block cursor-pointer bg-yellow-400"
                >
                  {companyArabicName}
                </span>
              );
